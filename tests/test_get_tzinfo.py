...
from _get_tzinfo import SystemTZ
...
if TYPE_CHECKING:
    ...
    from time import _TimeTuple as TimeTuple, struct_time
...
def winkle_tz_out_of_zoneinfo(
    zi: ZoneInfo,
) -> tuple[int, int, int, tuple[str | None, str | None]]:
    ...
    log: list[tuple[int, int | None, str | None, str | None]] = []
...
def tzset() -> Iterator[Callable[[], None]]:
    ...
    import tzlocal.utils  # isort: skip
    ...
    # cwd = Path.cwd()
    # tzpaths = (Path(p) for p in sys.path)
    # tzpaths = (p for p in tzpaths if p.is_absolute() and p.is_dir() and not p.samefile(cwd))
    # tzpaths = (p / s / 'zoneinfo' for s, p in product(['tzdata'], tzpaths))
    # tzpaths = [p for p in tzpaths if p.is_dir()]
    # reset_tzpath(tzpaths)
    ...
    if orig_tzset:
        ...
        def emulated_localtime(zi: ZoneInfo, secs: float | None = None) -> struct_time:
            if secs is None:
                secs = time.time()
        ...
        @wraps(time.mktime)
        def emulated_mktime(zi: ZoneInfo, t: struct_time | TimeTuple) -> float:
            candidate_ts = None
...
POINTS_IN_TIME: dict[
    str, tuple[str, int, tuple[int, ...], int, int, str, dict[str, int]] | ParameterSet
] = {
    # fmt: off
...
def test_timezone_correctly_applied(
    key: str,
    ts: int,
    tt: tuple[int, ...],
    off: int,
    ...
    with welldefined_timezone(key):
        assert os.environ.get('TZ') == key
        dt = DateTime.now(tz=systemtz)
        assert dt.tzname() in time.tzname
...
def test_datetime(
    ...
    with welldefined_timezone(key) as zi:
        dt = DateTime(*tt, tzinfo=zi, fold=kwds.get('fold', 0))
...
def test_systemtz(
    ...
    with welldefined_timezone(key):
        dt = DateTime(*tt, tzinfo=systemtz, fold=kwds.get('fold', 0))
...
def test_systemtz_fromtimestamp(
    key: str,
    ts: int,
    tt: tuple[int, ...],
    off: int,
    ...
    with welldefined_timezone(key):
        dt = DateTime.fromtimestamp(ts, tz=systemtz)
        assert isinstance(dt.tzinfo, SystemTZ)
        expected_fold = kwds.get('fold', 0)
        assert dt.fold == expected_fold
        assert (dt.year, dt.month, dt.day, dt.hour, dt.minute, dt.second) == tt
...
def test_time_localtime(
    ...
    with welldefined_timezone(key):
        ...
        assert st[:6] == tt
        assert st.tm_isdst == int(bool(dst))
        assert st.tm_zone == zone
        assert st.tm_gmtoff == off
...
def test_time_mktime(
    ...
    with welldefined_timezone(key):
        st = time.struct_time((*tt, 99, 999, int(bool(dst))), {'tm_zone': zone, 'tm_gmtoff': off})
        assert time.mktime(st) == ts
